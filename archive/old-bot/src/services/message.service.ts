...
const allowedRaidChannels: string[] = [
  ...
  ChannelIds.RaidIzegem.toString(),
];
...
export class MessageService implements IMessageService {
  ...
  get commandArguments() {
    return this.message!.content.substring(
      ['!', '?'].some((x) => x === this.commandSymbol) ? 1 : 0
    )
      .split(' ')
      .filter(Boolean);
  }
  ...
  async handleRaidStart() {
    ...
    if (allowedRaidChannels.some((x) => x === this.message!.channel.id)) {
      const richEmbed = new RichEmbed()
        ...
        .setTimestamp()
        .setFooter(
          `${DiscordHelper.findDisplayName(this.message!)}`,
          `${DiscordHelper.findDisplayAvatar(this.message!)}`
        )
  ...
  async handleRankRequest() {
    ...
    if (!isNullOrUndefined(user) && !isNullOrUndefined(role)) {
      // if user exists
      if (
        isNullOrUndefined(role.members.filter((x) => x.id === user!.id).first())
      ) {
      ...
      try {
        ...
        this.message!.guild.members.get(user.id)!.setNickname(
          `${firstName}|${playerName}|${emoji}|${level}`
        );
      } catch (error) {
