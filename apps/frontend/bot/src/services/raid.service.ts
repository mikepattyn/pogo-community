...
export class RaidService {
  ...
  getUser(messageId: string, userId: string) {
    return this.getRaid(messageId).players.filter(
      (x: IPlayer) => x.id === userId
    )[0];
  }
  ...
  isValidAdditionEmoji(emoji: string) {
    return (
      additionsEmojis.filter((emojeh: string) => emojeh === emoji).length === 1
    );
  }
  ...
  addPlayerAddition(messageId: string, userId: string, emoji: string) {
    this.getUser(messageId, userId).additions =
      additionsEmojis.indexOf(emoji) + 1;
  }
  addPlayerToRaid(reaction: MessageReaction, user: User) {
    this.getRaid(reaction.message.id).players.push(
      new Player(
        user.id,
        reaction.message.guild.members.get(user.id)!.displayName
      )
    );
  }
    ...
    try {
      ...
      if (date > now) {
        ...
        var raid = new Raid(
          ...
          [],
          date,
          new Player(startedBy.author.id, this.findDisplayName(startedBy))
        );
  ...
  async removeUserAdditionEmojis(reaction: MessageReaction, user: User) {
    ...
    if (
      additionsEmojis.indexOf(reaction.emoji.name) == -1 &&
      reaction.emoji.name != '👍'
    ) {
    ...
    if (
      ...
      reaction.message.reactions.forEach((reactie) => {
        if (
          dontDelete.filter((x) => x.emoji.name === reactie.emoji.name).values
            .length == 0
        ) {
          if (
            reactie.emoji.name != '👍' &&
            reactie.emoji.name != reaction.emoji.name
          ) {
            reactie.remove(user);
          }
        }
  ...
  findDisplayName(message: Message) {
    return message.guild.members.find((x) => x.id === message.author.id)
      .displayName;
  }
