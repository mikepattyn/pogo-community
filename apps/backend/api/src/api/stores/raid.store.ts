...
export class RaidStore {
    async getByMessageId(id: string) {
        try {
            return new Promise((resolve: any, reject: any) => {
                poolPromise.query(DataRaid.GetByMessageId(), [id],
    ...
    async addPlayer(raidId: string, playerId: string) {
        try {
            return new Promise((resolve: any, reject: any) => {
                poolPromise.query(DataRaid.AddPlayerToRaid(), [raidId, playerId, new Date()],
                    (error: any, results: any, fields: any) => {
                        if (error) reject(error);
                        if (results) {
                            resolve(results)
    ...
    async post(dataRaid: IDataRaid) {
        try {
            return new Promise((resolve: any, reject: any) => {
                poolPromise.query(DataRaid.Insert(), [dataRaid.MessageId, dataRaid.GymId, dataRaid.Tiers, dataRaid.TimeRemaining],
                    (error: any, results: any, fields: any) => {
                        if (error) reject(error);
                        if (results) {
