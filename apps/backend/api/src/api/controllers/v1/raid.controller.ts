...
export class raidController extends BaseHttpController implements interfaces.Controller {

    constructor(@inject(RaidStore) private raidStore: RaidStore) { super() }

    @httpGet('/:id')
    private async getByMessageId(@requestParam("id") id: string, @response() res: express.Response) {
        if (await this.httpContext.user.isAuthenticated()) {
            await this.raidStore.getByMessageId(id)
                .then((result) => {
                    console.log(result[0])
                    var location: ILocation = new Location({ Longtitude: result[0].Longtitude, Latitude: result[0].Latitude })
                    var gym: IGym = new Gym({ Name: result[0].Name, Location: location })
                    var raid: Raid = new Raid(result[0].Pokemon, gym, result[0].Tiers, result[0].TimeRemaining);
    ...
    private async post(@request() req: express.Request, @response() res: express.Response) {
        if (await this.httpContext.user.isAuthenticated()) {
            await this.raidStore.post(req.body)
                .then(() => {
                    res.sendStatus(201)
                })
                .catch((error: any) => {
    ...
    @httpPost('/:id/players')
    private async addPlayer(@request() req: express.Request, @response() res: express.Response) {
        if (await this.httpContext.user.isAuthenticated()) {
            await this.raidStore.addPlayer(req.params.id, req.body.PlayerId)
                .then(() => {
                    res.sendStatus(201)
                })
                .catch((error: any) => {
